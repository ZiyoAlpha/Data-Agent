...
聊天接口会先运行确定性的授权判断。只有明确的肯定表达才进入写入流程，例如：

- “帮我把刚才的结论沉淀到知识库。”
- “请将这段规则写入知识库。”
- “Save this definition to the knowledge base.”

“不要沉淀”“不用保存到知识库”等否定表达保持只读。触发后的流程是：

1. OpenAI Structured Outputs 把当前请求和最近对话整理成单篇知识草稿。
2. 草稿包含 `ready`、分类、slug、标题、摘要、正文、来源、状态和缺失信息。
3. `ready=false` 时只向用户追问，不创建文件。
4. `ready=true` 时交给本地写入器再次验证并创建文件。
5. 同名路径存在时返回 `confirmation_required`，不会静默覆盖。
6. 只有“确认覆盖知识库文档 <明确相对路径>”才能执行覆盖；模型本身无权开启覆盖。
7. 前端显示写入状态和相对路径，成功后刷新已索引文档数。

这是“明确要求才沉淀”，不是后台自动记忆，也不会把普通回答自动保存。
